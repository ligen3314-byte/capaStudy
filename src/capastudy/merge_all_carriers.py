...
def save_merged(voyages: pd.DataFrame, port_calls: pd.DataFrame, selected: Dict[str, Path]) -> Path:
    ...
    # Backward compatibility: keep a copy in legacy merged_query.
...
def merge_current_entity(
    ...
    new_core = new_df.copy()
    cur = ensure_audit_columns(current_df.copy())

    # Keep exactly one row for each key in new snapshot.
    new_core = new_core.drop_duplicates(subset=[key_col], keep="first").copy()
    ...
    inserts = sorted(new_keys - cur_keys)
    common = sorted(new_keys & cur_keys)
    disappears = sorted(cur_keys - new_keys)
    ...
    # Put key and audit fields near the front for easier review.
...
def save_update_outputs(
    voyages_new: pd.DataFrame,
    port_calls_new: pd.DataFrame,
    selected: Dict[str, Path],
) -> Dict[str, Path]:
    ...
    # Build row-level key for port calls so update/disappear can be tracked.
    port_calls_new = port_calls_new.copy()
    port_calls_new["portcall_key"] = build_portcall_key(port_calls_new)

    voyages_updated, v_changes = merge_current_entity(
        entity_name="voyages",
        new_df=voyages_new,
        current_df=voyages_cur,
        key_col="voyage_id",
        snapshot_date=snapshot_date,
        updated_at=updated_at,
    )
    portcalls_updated, p_changes = merge_current_entity(
        entity_name="portcalls",
        new_df=port_calls_new,
        current_df=portcalls_cur,
        key_col="portcall_key",
        snapshot_date=snapshot_date,
        updated_at=updated_at,
    )

    source_df = pd.DataFrame(
        [{"Carrier": c, "SourceFile": p.name, "SourcePath": str(p)} for c, p in selected.items()]
    )
    ...
    # Append daily snapshot into history workbook.
    ...
    # Keep a full raw snapshot for quick rollback/audit.
    ...
    # Backward compatibility: mirror update outputs to legacy path.
